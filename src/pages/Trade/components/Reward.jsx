...
const Reward = (props) => {
   ...
   const [pendingReward, setPendingReward] = useState(props.pendingReward)
  ...
  const [showUnStake, setShowUnStake] = useState(false)
   ...
   const toClaim = useCallback(async () => {
      if(loading || props.pendingReward<=0) {
         return
      }
      setLoading(true)
      claim().then(res => {
   ...
   const toApprove = useCallback(()=>{
      try {
        setLoading(true)
        approve(findAddressByName('Orich-ETH'), 
   ...
   useEffect(async () => {
      try {
         ...
         if (props.account) {
            ...
            let bal = await getBalance(props.account, findAddressByName('Orich-ETH'))
            setBal(toFixed(fromUnit(bal), 4))
            setBalanceLoading(false)
            let allowance_amount = await allowance(findAddressByName('Orich-ETH'), findAddressByName('RewardUnlocker'))
   ...
   return (
      <div className='w100 reward-modal p-t-24'>
         ...
         <div className="fz-24 fwb c2b ta m-t-5"><Activenumber value={fromUnit(pendingReward)} decimals={4} /> $Orich</div>
         <div className="staked w100 p-t-18 p-b-18 p-l-16 p-r-16 m-t-24">
            <div className="flex flex-between flex-center">
               ...
               <span className='fz-14 fwb c2b'>
                  ...
                  <img src={getTokenByName('ETH').icon} alt="" className='token-icon right-icon' />
                  <span className='m-l-5'>
                  <Activenumber value={fromUnit(depositAmount)} decimals={4} /> Orich-ETH LP
                  </span>
               </span>
            </div>
                  ...
                  <>
                     <div className="flex flex-between m-t-28">
                        <span className='fz-14 c236'>
                           <img src={getTokenByName('Orich').icon} alt="" className='token-icon' />
                           <img src={getTokenByName('ETH').icon} alt="" className='token-icon right-icon' />
                           <span className='m-l-5'>Orich-ETH LP</span>
                        </span>
