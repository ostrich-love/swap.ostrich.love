...
export const queryPoolInfo = async(depositCoinType, rewardCoinType) =>{
    return await queryModuleResource(
        ...
        `Pool<${depositCoinType},${rewardCoinType}>`,
        moduleType
    )
}
export const getPoolPendingRewardIndex = async (depositCoinType, rewardCoinType)=> {
    ...
    if (last_distribute_time > 0) {
        ...
        let reward_per_bond = reward.times(INDEX_PRECISION).div(new BN(pool.total_deposit_weight));
...
export const queryDepositInfo = async(user, depositCoinType, rewardCoinType) => {
    return await queryModuleResource(user, `UserStore<${depositCoinType},${rewardCoinType}>`,moduleType);
}
export const getPendingReward = async(user, depositCoinType, rewardCoinType)=> {
    let user_store = (await queryDepositInfo(user, depositCoinType, rewardCoinType))?.data;
    if(!user_store) {
        return 0
    }
    let pending_reward = user_store.pending_reward ?Number(user_store.pending_reward):0;
    let pool_reward_index = await getPoolPendingRewardIndex(depositCoinType, rewardCoinType);
    let pending_rewards = []
    for (let deposit_info of user_store.depositions) {
        ...
        let pending = deposit_mount
            .times(pool_reward_index.minus(deposit_reward_index))
            .div(INDEX_PRECISION)
            .toNumber();
        pending_reward += (Number(deposit_info.pending_reward*1) + Number(pending))
        pending_rewards.push((Number(deposit_info.pending_reward*1) + Number(pending)))
        console.log("pending", pending_reward);
    }
    // console.log("pool_reward_index", pool_reward_index.toNumber());

    return {pending_reward,pending_rewards};
}
export const getPendingRewardByIndex = async(user, depositCoinType, rewardCoinType, index)=> {
    let user_store = (await queryDepositInfo(user, depositCoinType, rewardCoinType))?.data;
    if(!user_store) {
        return 0
    }
    let pending_reward = user_store.pending_reward ?Number(user_store.pending_reward):0;
    let pool_reward_index = await getPoolPendingRewardIndex(depositCoinType, rewardCoinType);
    let deposit_info = user_store.depositions[index]
    let deposit_mount = new BN(deposit_info.amount);
    let deposit_reward_index = new BN(deposit_info.reward_index);
    // console.log("deposit_reward_index", deposit_reward_index.toNumber());
    let pending = deposit_mount
...
export const getApr = async (info) => {
    ...
    if(info.lptype == 'LPToken') {
        ...
        apr_temp = new BigNumber(info.rewards_per_week).multipliedBy(new BigNumber(priceHarw)).multipliedBy(new BigNumber(5200)).multipliedBy(new BigNumber(11.2)).dividedBy(info.total_deposit_weight).dividedBy(lpprice).toString()
        return apr_temp
      } else {
        apr_temp = new BigNumber(info.rewards_per_week).multipliedBy(new BigNumber(5200)).multipliedBy(new BigNumber(11.2)).dividedBy(info.total_deposit_weight).toString()
         return apr_temp
      }
}
...
export const calApr = async (info, weeks, amount=1) => {
    ...
    if(info.lptype == 'LPToken') {
        ...
        // let deposit_value = (info.total_deposits/lptotal)*reserve_y*2
        let lpprice = reserve_y*2/lptotal
        apr_temp = new BigNumber(info.rewards_per_week).multipliedBy(new BigNumber(priceHarw)).multipliedBy(new BigNumber(5200)).multipliedBy((new BigNumber(0.2).multipliedBy(weeks).plus(new BigNumber(0.8)))).dividedBy(new BigNumber(info.total_deposit_weight).plus((new BigNumber(0.2).multipliedBy(weeks).plus(new BigNumber(0.8))).multipliedBy(new BigNumber(amount)))).dividedBy(lpprice).toString()
 
        return apr_temp
      } else {
        apr_temp = new BigNumber(info.rewards_per_week).multipliedBy(new BigNumber(5200)).multipliedBy((new BigNumber(0.2).multipliedBy(weeks).plus(new BigNumber(0.8)))).dividedBy(new BigNumber(info.total_deposit_weight).plus((new BigNumber(0.2).multipliedBy(weeks).plus(new BigNumber(0.8))).multipliedBy(new BigNumber(amount)))).toString()
